...
import {
  SetVoteRequest,
  SignData,
} from '@crypto-mayhem-frontend/crypto-mayhem/data-access/dao-model';
...
export class DAOEffects {
  ...
  showDaoSmallSpinnerActive$ = createEffect(() =>
    this.actions$.pipe(
      ofType(WalletActions.connectWallet),
      map(() => DAOActions.showDaoSmallSpinnerActive())
    )
  );

  hideDaoSmallSpinnerActive$ = createEffect(() =>
    this.actions$.pipe(
      ofType(DAOActions.getAllActiveTopicsSuccess),
      map(() => DAOActions.hideDaoSmallSpinnerActive())
    )
  );
  ...
  showDaoLargeSpinnerActive$ = createEffect(() =>
    this.actions$.pipe(
      ...
      map(() => DAOActions.showDaoLargeSpinnerActive())
    )
  );
  ...
  hideDaoLargeSpinnerActive$ = createEffect(() =>
    this.actions$.pipe(
      ...
      map(() => DAOActions.hideDaoLargeSpinnerActive())
    )
  );

  showDaoSmallSpinnerHistoric$ = createEffect(() =>
    this.actions$.pipe(
      ofType(WalletActions.connectWallet),
      map(() => DAOActions.showDaoSmallSpinnerHistoric())
    )
  );

  hideDaoSmallSpinnerHistoric$ = createEffect(() =>
    this.actions$.pipe(
      ofType(DAOActions.getAllHistoricTopicsSuccess),
      map(() => DAOActions.hideDaoSmallSpinnerHistoric())
    )
  );

  showDaoLargeSpinnerHistoric$ = createEffect(() =>
    this.actions$.pipe(
      ofType(
        WalletActions.connectWalletSuccess,
      ...
      map(() => DAOActions.showDaoLargeSpinnerHistoric())
    )
  );

  hideDaoLargeSpinnerHistoric$ = createEffect(() =>
    this.actions$.pipe(
      ofType(
        DAOActions.getAllHistoricTopicsSuccess,
        DAOActions.postDaoVoteWithSignatureSuccess
      ),
      map(() => DAOActions.hideDaoLargeSpinnerHistoric())
    )
  );
    ...
    this.actions$.pipe(
      ...
      switchMap(([, wallet, lang]) =>
        this.daoService.getDaoAllActiveTopics(wallet, lang).pipe(
          ...
          catchError((error) =>
            of(DAOActions.getAllActiveTopicsFailure({ error }))
          )
        )
      )
    )
  ...
  getAllHistoricTopics$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(
        (
          [action, wallet, localization, storeTopic] //TODO SKIP AND TAKE to improve for pagination - 0 - 20 per page.
        ) =>
          this.daoService
            .getAllHistoricTopics(wallet, 0, 25, localization)
            .pipe(
  ...
  signMessageForDao$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(({ messages }) =>
        from(this.walletService.signMessage(messages)).pipe(
          map((dataAndSignedMessage) =>
            DAOActions.signMessageForDaoSuccess({ dataAndSignedMessage })
          )
        )
      )
    )
